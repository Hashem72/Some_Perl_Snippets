...
########################### Note ########################################################
#     Main difference between V2 and original one is that in V2 we are trying to learn  #
# background model by movine features either left or right depending on their strands,  #
# whereas in origial one we were trying to put for example p(a)=1/4                     #
...
my @matrix_of_compositions_of_real_tags = &GET_PWM($chr_seq,$tags_input_file,$output_file_name,36,$offset,$prior_probability);



my @matrix_of_compositions_of_moved_tags = &GET_PWM($chr_seq,$tags_input_file,$output_file_name,36,$offset,$prior_probability,10);


&GET_BIASNESS_SCORE_FOR_TAGS($chr_seq,$tags_input_file,$output_file_name,\@matrix_of_compositions_of_real_tags,\@matrix_of_compositions_of_moved_tags,36);
exit;
...
sub GET_BIASNESS_SCORE_FOR_TAGS($$$$$$){
    ...
    my $pms_file = "/nfs/th_group/hk3/UW_DNaseI_HS/Gm12878/pms_scores.txt";
    #prob of model given background
    my $pbs_file = "/nfs/th_group/hk3/UW_DNaseI_HS/Gm12878/pbs_scores.txt";
...
sub GET_PWM($$$$$$$){
    ...
    my $prior_prob         = shift;
    my $transfromin_length = shift;

    $transfromin_length =0 unless defined($transfromin_length);
	...
	if($strand eq "+"){
	    $start = $start +   $transfromin_length;
	    $end   = $end  +    $transfromin_length;
	}
	elsif($strand eq "-"){
	    $start = $start -  $transfromin_length;
	    $end   = $end   -  $transfromin_length;
...
#rid of gettin ln of zero.
    ...
    my $pwm_testing_file = "/nfs/th_group/hk3/UW_DNaseI_HS/Gm12878/pwm_bg_model_testing_file.txt";
